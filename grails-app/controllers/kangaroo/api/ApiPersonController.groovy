...
class ApiPersonController extends ApiBaseController {

    def list = {
        def faculty = Professor.findAllByIsProfessor(true).collectEntries { [it.id, it] }.sort()
        def staff = Professor.findAllByIsProfessor(false).collectEntries { [it.id, it] }.sort()

        render([faculty: faculty, staff: staff] as JSON)
    }
